...
this.setData({loading:true});try{const response=await this.list.read<PageResult<MenuItem>>('/api/menu/unfinished',{},append);if(response)this.setData({items:response.data.items,nextCursor:response.data.nextCursor,listTotal:response.data.total})}catch(e){showError(e)}finally{this.setData({loading:this.list.loading})}},
